...
public class PlayerService {
    ...
    //修改玩家最近活跃时间
    @Transactional
    public boolean updateStatus(String playerId,String status) {
        int rows = playerDao.updateStatus(playerId, status);
        if (rows == 0) {
            return false;
        }
        return true;
    ...
    public PlayerStatus getPlayerStatus(String playerId) {
        return playerDao.getPlayerStatus(playerId);
    }
}
